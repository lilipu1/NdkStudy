...
ResvFrameEnd(lame_internal_flags * gfc, int mean_bits)
{
    ...
    /* NOTE: enabling the NEW_DRAIN code fixes some problems with FhG decoder
       ...
       old drain code:
             include.lame -b320 BlackBird.wav ---> does not play with GraphEdit.exe using FhG decoder V1.5 Build 50

       new drain code:
             include.lame -b320 BlackBird.wav ---> plays fine with GraphEdit.exe using FhG decoder V1.5 Build 50
