...
/* decode only, use include.lame/mpglib to convert mp3 to wav */
int
...
lame_set_mode(lame_global_flags * gfp, MPEG_mode mode)
{
    if (is_lame_global_flags_valid(gfp)) {
        int     mpg_mode = mode;
        /* default: include.lame chooses based on compression ratio and input channels */
 ...
 *   0 = default = include.lame chooses
 *  -1 = disabled
 */
int
...
in include.lame.h.  You should not use them, they are experimental and may
change.  
